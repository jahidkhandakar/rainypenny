...
abstract final class DemoDataset {
  ...
  static DateTime _inCycle(
    ...
    final position = spread <= 0
        ? elapsed
        : (((spread - daysAgo) / spread) * elapsed).round();

    return cycle.start
        .add(Duration(days: position.clamp(0, elapsed)))
  ...
  static List<Transaction> transactionsFor({int payday = 1}) {
    final rows = <Transaction>[
      ...
      Transaction(
        ...
        title: 'Monthly Salary',
        amount: 3500.00,
        date: _salaryDay(payday, cyclesBack: 1),
        type: TransactionType.income,
        category: salary,
        note: 'Northwind Design Studio',
      ),
  ...
  static List<Transaction> _anchorToCycles(
  ...
  static List<Loan> get loans => [
    Loan(
      ...
      interestRate: 5.9,
      icon: CategoryIcon.transport,
      // A 36-month plan a little over a third of the way through, so the
      // demo shows a real term counting down rather than an empty one.
      totalInstallments: 36,
      paidInstallments: 14,
      startDate: DateTime(today.year - 1, today.month, 5),
    ),
    Loan(
      id: 'loan-card',
      name: 'Credit Card',
